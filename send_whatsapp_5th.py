...
# Setup logging
logging.basicConfig(
    filename='logs/whatsapp_log_5th.txt',
    level=logging.INFO,
...
Now, we’re scaling this up for the upcoming August 28th Mid Exams, and we’re onboarding a few standout 5th  semester students as Subject Experts.
Based on your 1st and 2nd  year performances, we believe you'd be a perfect fit. This is a paid opportunity + a chance to be a core part of something big at GITAM! 🌟
...
def send_whatsapp_message(name, number, message, wait_time=15):
    try:
        ...
        logging.info(f"5th sem | {name} | {number} | Message sent")
        return True
    
    except Exception as e:
        logging.error(f"5th sem | {name} | {number} | Failed to send message: {e}")
        return False
...
def process_csv(file_path):
    ...
    for idx, row in df.iterrows():
        ...
        log_msg = f"5th sem | {name} | {number} | {'Success' if success else 'Failed'}"
        print(log_msg)
        time.sleep(5)  # To avoid being flagged as spam

if __name__ == "__main__":
    print("Starting WhatsApp automation for 5th semester...")
    process_csv("details/students_5th_sem.csv")
    print("Done. Check logs/whatsapp_log_5th.txt for details.") 